...
READ_COMMANDS = {"df", "du", "free", "lsblk", "blkid", "findmnt", "ps", "uptime", "uname", "id", "whoami", "lspci", "lsusb", "sensors", "stat", "ls", "apt-cache", "dpkg-query", "ping", "tracepath", "traceroute", "wc", "date", "which", "type"}
...
def classify_command(command: str, shell: bool = False) -> Decision:
    ...
    if shell:
        if re.search(r"\b(?:mkfs(?:\.[a-z0-9]+)?|wipefs|dd|shred|sfdisk|sgdisk|blkdiscard|rm|rmdir|unlink)\b|\|\s*(?:sudo\s+)?(?:ba)?sh\b|\bnft\s+(?:flush|delete)\b|\biptables\s+-(?:F|X|D)\b", command):
        ...
        if any(network_path(path) or protected_path(path) for path in redirects):
            return Decision(3, command, "Protected shell redirection may replace data")
    ...
    if name in {"python", "python3", "bash", "sh"} and rest in (["-V"], ["--version"]):
        return Decision(0, command, "Program version")
    if name in {"bash", "sh", "python", "python3", "perl", "ruby"}:
        return Decision(2, command, "Arbitrary program")
    if name in {"rm", "rmdir", "unlink", "shred"}:
        return Decision(3, command, "Remove files")
    if name in {"chmod", "chown", "chgrp"}:
        ...
        return Decision(2, command, "Change ownership or permissions")
    if name in {"iptables", "ip6tables", "nft", "ufw", "firewall-cmd", "netplan", "nmcli", "ifup", "ifdown", "route"}:
        if name == "nft" and rest and rest[0] in {"flush", "delete"} or name in {"iptables", "ip6tables"} and any(x in rest for x in ("-F", "--flush", "-X", "--delete-chain", "-D", "--delete")):
            return Decision(3, command, "Remove firewall rules")
        if name == "nft" and rest and rest[0] in {"list", "get", "describe"} or name in {"iptables", "ip6tables"} and any(x in rest for x in ("-L", "--list", "-S", "--list-rules")) or name == "ufw" and rest and rest[0] == "status":
            return Decision(0, command, "Firewall diagnostic")
        return Decision(2, command, "Network change may interrupt SSH; automatic rollback unavailable")
    if name == "ip":
        if any(x in rest for x in ("del", "delete", "flush")):
            return Decision(3, command, "Remove network configuration")
        if any(x in rest for x in ("add", "replace", "set")):
            return Decision(2, command, "Network change may interrupt SSH; automatic rollback unavailable")
    ...
    if name == "systemctl":
        ...
        if any(any(word in x.lower() for word in ("ssh", "network", "firewall")) for x in rest):
            return Decision(2, command, "Service change may interrupt SSH; automatic rollback unavailable")
    ...
    if name in {"apt", "apt-get"}:
        ...
        if any(x in rest for x in ("openssh-server", "network-manager", "netplan.io", "ufw", "nftables", "iptables")):
            return Decision(3 if action in {"remove", "purge", "autoremove"} else 2, command, "Package may affect remote access")
        return Decision(1 if action in {"install", "update", "upgrade"} else 3 if action in {"remove", "purge", "autoremove"} else 2 if action in {"full-upgrade", "dist-upgrade"} else 0 if action in {"search", "show", "list"} else 2, command, "Package operation")
    if name == "docker":
        action = next((x for x in rest if not x.startswith("-")), "")
        return Decision(0 if action in {"ps", "info", "inspect", "logs", "images", "stats", "version"} else 3 if action in {"rm", "rmi", "prune"} else 1 if action in {"pull", "start", "stop", "restart"} else 2, command, "Docker operation")
    if name == "journalctl":
    ...
    if name == "ethtool":
        return Decision(0 if len(rest) == 1 or len(rest) == 2 and rest[0] in {"-i", "-S", "-k", "-a"} else 2, command, "Network adapter operation")
    if name == "smartctl":
        return Decision(0 if len(rest) == 2 and rest[0] in {"-a", "-H", "-i", "-x"} else 2, command, "SMART operation")
    if name == "iw":
        return Decision(0 if not any(x in rest for x in ("connect", "disconnect", "set", "del", "add")) else 3 if "del" in rest else 2, command, "Wi-Fi operation")
    ...
    if name in {"mkdir", "touch", "cp", "mv", "tee"}:
        return Decision(3 if name == "mv" else 2 if any(network_path(x) for x in rest if x.startswith("/")) else 1, command, "File change")
...
def classify(tool: str, args: dict) -> Decision:
    if tool == "create_tool":
        digest = hashlib.sha256(args["source"].encode()).hexdigest()[:16]
        return Decision(2, f"{args['name']} sha256:{digest}", "Register arbitrary Python tool with current OS privileges")
    if tool == "inspect_tool":
        return Decision(0, args.get("name", "all generated tools"), "Inspect generated tool")
    from .custom_tools import get as get_custom_tool
    if get_custom_tool(tool):
        return Decision(2, f"{tool} {args}", "Run generated Python tool with current OS privileges")
    if tool == "shell_exec":
    ...
    if tool == "service_manager":
        ...
        level = 0 if action in {"status", "is-active", "is-enabled"} else 2 if service in {"ssh", "sshd", "NetworkManager", "systemd-networkd", "networking"} else 1
        description = f"systemctl {action}" + ("; SSH may disconnect; automatic rollback unavailable" if level == 2 else "")
    ...
    if tool == "package_manager":
        ...
        level = 0 if action in {"search", "query"} else 3 if action == "remove" else 2 if args.get("package", "") in critical else 1
    ...
    if tool == "docker":
        action = args["action"]
        return Decision(0 if action in {"info", "ps", "inspect", "logs", "images", "volumes"} else 3 if action == "remove" else 2 if action == "run" else 1, args.get("name", args.get("image", "docker")), f"docker {action}")
    if tool in {"write_file", "edit_file"}:
        ...
        level = 2 if network_path(path) or path.startswith(("/etc/", "/boot/")) else 1
        return Decision(level, path, f"File {tool}" + ("; SSH may disconnect; automatic rollback unavailable" if network_path(path) else ""))
...
def approve(decision: Decision, *, dry_run: bool, interactive: bool) -> bool:
    if dry_run or decision.level < 3:
        return True
    if not interactive:
        return False
